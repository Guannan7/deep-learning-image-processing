...
模型简介:
    YOLOv8s 是 Ultralytics 公司于2023年发布的 YOLOv8 系列中的小型模型。
    它是YOLOv8系列中最常用的中型模型，在精度和速度之间取得了很好的平衡。
...
使用示例:
    python yolov8s_detector.py --image input.jpg
    python yolov8s_detector.py --video input.mp4 --show --device cuda
"""
...
class YOLOv8sDetector:
    ...
    def __init__(self, model_path: str = 'yolov8s.pt', conf_threshold: float = 0.25,
    ...
    def load_model(self) -> bool:
        ...
        """
        try:
            logger.info(f"正在加载YOLOv8s模型: {self.model_path}")
    ...
    def draw_results(self, image: np.ndarray, results: Results,
                     show_confidence: bool = True, show_label: bool = True,
...
def parse_args():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description='YOLOv8s 目标检测')
...
def main():
    ...
    # 视频检测模式
        ...
        while cap.isOpened():
            ...
            if args.show:
                cv2.imshow('YOLOv8s Detection', output_frame)
